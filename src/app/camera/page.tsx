...
function getAvailableResolutions(
  ...
  const unique = resolutions.filter(
    (res, index, arr) =>
      arr.findIndex((r) => r.width === res.width && r.height === res.height) ===
      index
  );
...
export default function CameraPage() {
      ...
      try {
        ...
        if (mounted) {
          ...
          if (errorName === "NotReadableError") {
            ...
            setErrorDetails(
              "The camera might be in use by another application or not properly connected."
            );
          ...
          if (track.readyState === "ended" || !track.enabled) {
            ...
            if (video.paused || video.ended || video.readyState < 2) {
              ...
              try {
                ...
                console.warn(
                  "Could not resume playback, restarting stream:",
                  playErr
                );
                ...
                if (playErr instanceof Error && playErr.name !== "AbortError") {
                  try {
                    ...
                    console.error(
                      "Failed to restart camera after play failure:",
                      restartErr
                    );
                  }
                }
  ...
  const handleSettingChange = async (
    ...
    // When exposure compensation is changed, switch to continuous mode (EV only works in auto mode)
    if (
      key === "exposureCompensation" &&
      capabilities?.exposureMode?.includes("continuous")
    ) {
      newSettings.exposureMode = "continuous";
      // Clear manual exposure settings when switching to auto with EV
  ...
  if (error) {
    ...
    const currentLensIndex = cameras.findIndex(
      (c) => c.deviceId === selectedCamera
    );
    const currentLensName =
      currentLensIndex >= 0 ? `Lens ${currentLensIndex + 1}` : "Camera";
    // Get next lens info for switch button
    const nextLensIndex =
      cameras.length > 1 ? (currentLensIndex + 1) % cameras.length : -1;
    const nextLensName =
      nextLensIndex >= 0 ? `Lens ${nextLensIndex + 1}` : "Next Camera";
  ...
  return (
    <main
      className={`relative h-dvh w-screen overflow-hidden bg-black text-white transition-all duration-300 ease-out ${
      ...
      {!isLandscape && (
        <div className="absolute left-0 right-0 top-0 p-3 transition-all duration-300 ease-out transform">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() =>
                setActiveControl(activeControl === "lens" ? null : "lens")
              }
            ...
            >
              ...
              <span>
                Lens{" "}
                {cameras.findIndex((c) => c.deviceId === selectedCamera) + 1}
              </span>
            </button>{" "}
            <div className="flex items-center gap-2">
              ...
              {capabilities?.aspectRatio && (
                <button
                  onClick={() =>
                    setActiveControl(
                      activeControl === "aspectRatio" ? null : "aspectRatio"
                    )
                  }
                ...
                >
                  <div className="flex h-6 w-8 items-center justify-center rounded border border-white/30 bg-white/10">
                    <svg
                      className="h-4 w-6 text-white/70"
                      viewBox="0 0 24 16"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="1.5"
      ...
      {isLandscape && (
        <div className="absolute left-0 right-0 top-2 flex w-full items-center justify-between gap-2 px-2 py-2 transition-all duration-300 ease-out transform">
          ...
          >
            ...
            <span>
              Lens {cameras.findIndex((c) => c.deviceId === selectedCamera) + 1}
            </span>
          </button>
          ...
          <div className="flex items-center gap-3">
            ...
            {capabilities?.iso && (
              <button
                onClick={() =>
                  setActiveControl(activeControl === "iso" ? null : "iso")
                }
                className={`flex h-12 w-12 flex-col items-center justify-center rounded-xl transition-all ${
                  activeControl === "iso"
                    ? "bg-blue-600"
            ...
            {capabilities?.colorTemperature && (
              <button
                onClick={() =>
                  setActiveControl(
                    activeControl === "colorTemperature"
                      ? null
                      : "colorTemperature"
                  )
                }
            ...
            {capabilities?.exposureCompensation && (
              <button
                onClick={() =>
                  setActiveControl(
                    activeControl === "exposureCompensation"
                      ? null
                      : "exposureCompensation"
                  )
                }
            ...
            {capabilities?.zoom && (
              <button
                onClick={() =>
                  setActiveControl(activeControl === "zoom" ? null : "zoom")
                }
                className={`flex h-12 w-12 flex-col items-center justify-center rounded-xl transition-all ${
                  activeControl === "zoom"
                    ? "bg-blue-600"
            ...
            {capabilities?.focusDistance && (
              <button
                onClick={() =>
                  setActiveControl(
                    activeControl === "focusDistance" ? null : "focusDistance"
                  )
                }
                className={`flex h-12 w-12 flex-col items-center justify-center rounded-xl transition-all ${
                  activeControl === "focusDistance"
                    ? "bg-blue-600"
                    : "bg-black/50 backdrop-blur-sm"
                }`}
              >
                {settings.focusMode === "manual" ? (
                  <>
                    ...
                    <span className="text-xs font-semibold">
                      {(
                        settings.focusDistance ?? capabilities.focusDistance.min
                      ).toFixed(1)}
                    </span>
                  </>
                ) : (
            ...
            {capabilities?.exposureTime && (
              <button
                onClick={() =>
                  setActiveControl(
                    activeControl === "exposureTime" ? null : "exposureTime"
                  )
                }
                className={`flex h-12 w-12 flex-col items-center justify-center rounded-xl transition-all ${
                  activeControl === "exposureTime"
                    ? "bg-blue-600"
                    : "bg-black/50 backdrop-blur-sm"
                }`}
              >
        ...
        <>
          ...
          <div
            className={`fixed z-50 left-0 right-0 bg-black/90 backdrop-blur-xl transition-all duration-300 ease-out ${
              isLandscape ? "bottom-0 rounded-t-3xl" : "top-0 rounded-b-3xl"
            }`}
            style={{
            ...
            <div className="px-6 pb-6 pt-2">
              ...
              {activeControl === "iso" && capabilities?.iso && (
                <div className="flex flex-col items-center gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => {
                        ...
                        // Always set exposure mode to continuous when clearing ISO to enable auto
                        if (capabilities?.exposureMode) {
                          if (
                            capabilities.exposureMode.includes("continuous")
                          ) {
                            newSettings.exposureMode = "continuous";
                          } else if (capabilities.exposureMode.length > 0) {
                            newSettings.exposureMode =
                              capabilities.exposureMode[0];
                          }
                        }
                        ...
                        if (stream) {
                          (async () => {
                            try {
                              ...
                              console.error(
                                "Failed to apply auto ISO settings:",
                                err
                              );
                            }
                          })();
                  ...
                  <div className="relative w-full max-w-md overflow-x-auto">
                    <div className="flex gap-2 px-2 pb-2">
                      {(() => {
                        // Standard photography ISO values
                        const standardISOValues = [
                          10, 12, 16, 20, 25, 32, 40, 50, 64, 80, 100, 125, 160,
                          200, 250, 320, 400, 500, 640, 800, 1000, 1250, 1600,
                          2000, 2500, 3200, 4000, 5000, 6400, 8000, 10000,
                          12800, 16000, 20000, 25600, 32000, 40000, 51200,
                          64000, 80000, 102400, 128000, 160000, 204800,
                        ];

                        // Filter to only ISO values within camera capabilities
                        const availableISOs = standardISOValues.filter(
                          (iso) =>
                            capabilities.iso &&
                        ...
                        return availableISOs.map((iso) => {
                          const isSelected = settings.iso === iso;
                          return (
                            <button
                              key={iso}
                              onClick={() => handleSettingChange("iso", iso)}
                    ...
                    <div className="flex flex-row items-center justify-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          ...
                          // Explicitly set white balance mode to auto/continuous
                          if (capabilities?.whiteBalanceMode) {
                            if (
                              ...
                              capabilities.whiteBalanceMode.length > 0
                            ) {
                              newSettings.whiteBalanceMode =
                                capabilities.whiteBalanceMode[0];
                            }
                          }
                          ...
                          if (stream) {
                            (async () => {
                              try {
                                ...
                                console.error(
                                  "Failed to apply auto WB settings:",
                                  err
                                );
                              }
                            })();
                  ...
                  <div className="flex flex-col items-center gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => {
                          const newSettings = { ...settings };
                          // EV's auto value is 0, not undefined
                          newSettings.exposureCompensation = 0;
                          setSettings(newSettings);
                          ...
                          if (stream) {
                            (async () => {
                              try {
                                ...
                                console.error(
                                  "Failed to apply auto EV settings:",
                                  err
                                );
                              }
                            })();
                    ...
                    <div className="relative w-full max-w-md overflow-x-auto">
                      <div className="flex gap-2 px-2 pb-2">
                        {(() => {
                          // Generate EV values in 0.5 stop increments
                          const evMin = Math.max(
                            capabilities.exposureCompensation.min,
                            -5
                          );
                          const evMax = Math.min(
                            capabilities.exposureCompensation.max,
                            5
                          );
                          const evValues = [];

                          for (let ev = evMin; ev <= evMax; ev += 0.5) {
                            evValues.push(ev);
                          }
                          ...
                          return evValues.map((ev) => {
                            const isSelected =
                              ...
                              Math.abs(settings.exposureCompensation - ev) <
                                0.1;
                            const label =
                    ...
                    <span className="text-xs text-white/60">
                      Scroll to see all EV values · ±0.5 stop increments
                    </span>
                  </div>
                )}
                ...
                capabilities?.focusDistance && (
                  <div className="flex flex-col items-center gap-3">
                    <div className="flex flex-row items-center justify-center gap-2">
                      ...
                      <button
                        onClick={() =>
                          handleSettingChange("focusMode", "continuous")
                        }
                ...
                capabilities?.exposureTime && (
                  <div className="flex flex-col items-center gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => {
                          ...
                          // Always set exposure mode to continuous when clearing exposure time to enable auto
                          if (capabilities?.exposureMode) {
                            if (
                              capabilities.exposureMode.includes("continuous")
                            ) {
                          ...
                          if (stream) {
                            (async () => {
                              try {
                                ...
                                console.error(
                                  "Failed to apply auto exposure time settings:",
                                  err
                                );
                              }
                            })();
                    ...
                    <div className="relative w-full max-w-md overflow-x-auto">
                      <div className="flex gap-2 px-2 pb-2">
                        {(() => {
                          // Standard photography shutter speeds in milliseconds
                          const standardShutterSpeeds = [
                            { ms: 0.125, label: "1/8000" },
                            { ms: 0.15625, label: "1/6400" },
                            { ms: 0.2, label: "1/5000" },
                            { ms: 0.25, label: "1/4000" },
                            { ms: 0.3125, label: "1/3200" },
                            { ms: 0.4, label: "1/2500" },
                            { ms: 0.5, label: "1/2000" },
                            { ms: 0.625, label: "1/1600" },
                            { ms: 0.8, label: "1/1250" },
                            { ms: 1, label: "1/1000" },
                            { ms: 1.25, label: "1/800" },
                            { ms: 1.5625, label: "1/640" },
                            { ms: 2, label: "1/500" },
                            { ms: 2.5, label: "1/400" },
                            { ms: 3.125, label: "1/320" },
                            { ms: 4, label: "1/250" },
                            { ms: 5, label: "1/200" },
                            { ms: 6.25, label: "1/160" },
                            { ms: 8, label: "1/125" },
                          ...
                          // Filter to only speeds within camera capabilities
                          const availableSpeeds = standardShutterSpeeds.filter(
                            (speed) =>
                              capabilities.exposureTime &&
                              speed.ms >= capabilities.exposureTime.min &&
                              speed.ms <= capabilities.exposureTime.max
                          );

                          return availableSpeeds.map(({ ms, label }) => {
                            const isSelected =
                              settings.exposureTime &&
                              Math.abs(settings.exposureTime - ms) < 0.5;
                            return (
                              <button
                                key={label}
                                onClick={() =>
                                  handleSettingChange("exposureTime", ms)
                                }
                                className={`px-4 py-2 rounded-lg transition-colors whitespace-nowrap flex-shrink-0 ${
                                  isSelected
                                    ? "bg-blue-600 text-white font-semibold"
                ...
                <div className="flex flex-col items-center gap-3">
                  <div className="flex flex-row items-center justify-center gap-2">
                    ...
                    <button
                      onClick={() =>
                        capabilities?.zoom &&
                        handleSettingChange("zoom", capabilities.zoom.min)
                      }
                      disabled={
                        !settings.zoom ||
